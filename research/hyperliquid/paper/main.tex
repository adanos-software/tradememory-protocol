...
for copy-trading blow-ups: most are sudden, a plain equity / drawdown stop fires
as early on them, and the residual behavioral signal (gradual over-leveraging)
precedes fewer than one blow-up in ten.
\end{abstract}
...
no authentication, the recent fill, order, and ledger history for any address
(subject to an $\approx$10{,}000-fill cap we return to in Section~\ref{sec:data}),
plus a coarse account-value history \citep{hyperliquid2024docs}. This is an unusually
transparent setting: positions and liquidations are settled on-chain, the
...
copy-stops; a representative class is the forex copy-trading risk guards that monitor
...
for cutting losses, consistent with the treatment of drawdown as a primary risk
measure \citep{magdon2004maxdd}. We previously read this as a defeat for behavioral
...
the operating point chosen on the tuning split only---before the sealed test data
were read---and event-clustered bootstrap
...
$1.0$. The labeling looks forward only to \emph{confirm} a crater (the 30-day
recovery and loss-confirmation checks); the labeled trigger time itself is the crater
onset, and because the detector is evaluated only strictly \emph{before} that time
under the guard band (Section~\ref{sec:method}), no label information leaks into the
detection window.
...
\emph{market-event} day when at least $3\%$ of the \emph{active} universe---masters
with an equity observation in the surrounding window---craters on that day, and
...
API throttle), plus a seed-fixed (address-hashed) sample of stable masters; the
resulting cohort is blow-up-heavy (2{,}482 blow-up / 973 stable, not a balanced
design). We do not balance the classes: FPR is reported on stable masters and recall
on blow-ups separately, with a rank-based AUC that is insensitive to the class ratio.
An address-hashed split assigns 1{,}405 masters to tuning, 656 to a validation split
used only for pre-lock development checks (it reports no numbers here), and a
...
collected, inferred, or published. We report only aggregate statistics and present
no address-level case study. No trading is performed and no capital is at
...
self-baseline}: the master's earliest contiguous \emph{healthy} prefix---the run of
buckets with equity $\geq 80\%$ of the running peak, ending at the first sustained
drop and capped at half the series. Both the running peak and the drop are computed
online from the prefix, so the baseline uses no future information. Masters with too
short a healthy prefix fall back to a short best-effort early window rather than being
excluded; we report this stability accounting below. To stabilize small-sample
baselines we shrink each master's per-axis location toward a tuning-split universe
distribution by James--Stein shrinkage \citep{james1961stein} with intensity $\kappa$.
...
per-axis level $\alpha = 0.01$ and a per-axis mixing scale $\tau$---the scale of the
mSPRT's mixing distribution over the drift effect size ($\tau = 0.3$ in the reported
configuration). The mSPRT is ``always valid'': checking it every
...
liquidation fill. Because alarms are scored only \emph{after} the burn-in baseline
window, the baseline-estimation segment (the earliest healthy buckets) and the
counted-alert region (after burn-in, up to 70\%-of-peak / first liquidation) do not
overlap in time.
...
We grid-search only two knobs on the tuning split---sustain $M \in \{2,3,4\}$ and
shrinkage $\kappa \in \{7,14\}$---holding the mixing scale $\tau=0.3$, 4-hour buckets,
and a 20-bucket burn-in fixed a priori. Among grid points with tuning-split stable-FPR
$\leq 0.10$ we take the highest-recall configuration, breaking ties by the longest
tuning lead. This selects $M=3$, $\kappa=7$ (with the fixed $\tau$, bucket size, and
burn-in)
...
\citep{efron1986bootstrap,cameron2008clusterboot}; in the blow-up-versus-stable AUC,
blow-ups are resampled by crash-day cluster and each stable master as its own
singleton cluster. We report the effective number of independent clusters as the true
power unit. Leave-one-event-out
...
buckets; 5 of the 2{,}482 fetched blow-ups fall short and drop from this panel) we
mark $T_{\text{behavior}}$, the first bucket after the early-window
baseline at which behavior clearly degrades---operationally, leverage exceeding
$1.5\times$ the baseline's 90th-percentile leverage, \emph{or} leverage above that
90th percentile while the master adds to a losing position, \emph{or} the stop-attach
rate falling below the baseline's 10th percentile---and $T_{\text{equity}}$, the first
bucket where equity falls below 90\% of its running peak. The lead is
$T_{\text{equity}} - T_{\text{behavior}}$ (positive when behavior leads); a master
with no such degradation has no $T_{\text{behavior}}$ and counts as \emph{sudden}.
...
``drift'' to detect. Over the 762 cases where both timestamps exist, the median lead
is \emph{negative} (\textbf{$-8$ days}): when behavior moves at all, it more often
...
\begin{figure}[t]
  ...
  lead over the 762 of 2{,}477 idiosyncratic blow-up masters with both a
  behavioral-degradation signal and a measurable equity crossing (model-free, no
  detector). A further 1{,}706 (69\%) show no behavioral signal at all---the
  \emph{sudden} blow-ups, which cannot be placed on this axis---and 9 have a signal but
  no equity crossing within the fill-capped behavioral window and are omitted
  ($762+1{,}706+9 = 2{,}477$). The mass at
  and below zero shows that behavioral drift does \emph{not} generally precede equity
  damage: 216 of all 2{,}477 (9\%) have a positive lead, and the median over the
  measurable cases is \emph{negative} ($\approx-8$\,d). The thin right tail is the
  gradual-leverage minority ($\approx$13-day median lead).}
  \label{fig:boundary}
\end{figure}

The boundary is clear: \emph{behavioral monitoring is not a general
...
blow-up from stable masters with $\mathrm{AUC}=0.49$ (event-clustered 95\% CI
$[0.47,\,0.51]$, straddling the $0.5$ chance line), and as a binary alarm it fires
early on only $2\%$ of blow-ups. The
...
\begin{figure}[t]
  ...
  point; the top-left corner (low FPR, high recall) is best. The detector sits at the far left (FPR 0.040) but
...
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item \textbf{Chance-level discrimination.} The continuous drift-evidence score
        ...
        ($\mathrm{AUC}=0.49$, 95\% CI $[0.47,\,0.51]$). The behavioral
...
Taken together, the detector does \textbf{not} discriminate: its $2\%$ recall and
chance-level AUC leave it silent on essentially all blow-ups, and its one favorable
property---a low false-positive rate---is of little operational value at this
coverage. This is the Claim-1 boundary, confirmed by the very machinery built to
beat it.
...
\begin{figure}[t]
  ...
  signal; it is silent on the rest. The signal here is the model-free drift rule
  (leverage / loser-add / stop-attach versus an early-window threshold), \emph{not}
  the mSPRT detector, whose median lead on its own fired subset is $\approx$49 days.}
  \label{fig:lead}
\end{figure}
...
than one blow-up in ten and rests on a coarse proxy. The result is a clear boundary:
behavioral early warning is not a general safety net for
copy-trading blow-ups; most are sudden, an equity stop fires directly when the damage
starts, and a behavioral layer would first need a causal, ahead-of-time definition of
the small drifting
...
detector form and the claim gates---in a timestamped version-control commit
(\texttt{573110e}) before the sealed test split was read once (Appendix~\ref{app:op}).
...
labeled time is the first genuinely loss-driven account-death. The rule uses
information after $t$ only to \emph{confirm} the crater (the 30-day recovery and
loss-confirmation checks); because the detector is evaluated strictly before the
crater onset under the guard band, no label information leaks into the detection
window. Robustness reruns vary the drawdown threshold over $\{0.50, 0.70, 0.85\}$.
