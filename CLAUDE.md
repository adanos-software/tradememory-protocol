...
- [2026-06-01] **FULL-UNIVERSE UPGRADE in progress (/goal, branch `copytrading-drift-demo`)** — upgrading the copy-trading drift paper from scaled (1.2k) to full-universe pre-registered. **Stage 0 DONE**: fetched full Hyperliquid leaderboard = **37,895 addrs** → `universe_addrs_full.txt` (no current-value pre-filter; `fetch_leaderboard.py`). **Stage 1 RUNNING** (bg, ~3.4h ETA): `stage1_full_universe.py` faithful pre-reg Part 1 (pre-T0 peak ≥$25k + ≥2 pre-T0 pts, forward-only loss-confirmed labeling, market-event τ=3%) with concurrent + global-rate-gate + JSONL checkpoint/resume. **KEY SCIENTIFIC FIX (Sean-approved Option B):** at locked T0=2025-06-01, equity-only craters are **65% withdrawals not losses** (median PnL/equity 0.205) — the pre-reg #6 "median 0.92" rationale was measured at a different T0 (2026-01-01 scoping). `blowup.forward_only_loss_confirmed_blowup`: a >70% crater counts only if loss-confirmed (PnL drop ≥0.5×equity drop); withdrawal craters reset peak + scan continues → first genuine loss-driven account-death. Validated 150-sample: blowup loss-ratio median 0.962, zero <0.5. **Stage 2/3/5 code pre-built** while Stage 1 runs: `cohort_behavioral_fetch_full.py` (uncap blowups + matched stable, concurrent+checkpoint, float-rounded), `fair_comparison.py`/`experiment_run.py` (+`--eval-split val/test` + `--lock-cfg` no-snooping firewall), `make_figures.py` (data-driven annotations + robust fig4). SHAs `01bc4cd` `bd1e634` `181769f`. **Next: Stage 1 done → Stage 2 fetch → Stage 3 experiments(val) → ⛔CONFIRM Sean→lock pre-reg Part 2→read test once → refresh paper → recompile → ping Sean.**
