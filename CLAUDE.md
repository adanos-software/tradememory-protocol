...
## Current Status
- **Copy-trading drift paper (2026-05-30)**: **Plan 2 Tasks 14+15 DONE (2026-05-31)**. `calibration.py` complete: `measure()`, `lda_weights()`, `grid_search()`, `pick_winner()`, `run_calibration()`. 135 tests green. Branch `copytrading-drift-demo`, HEAD `HEAD` (2 new commits: `519bd17` Task 14, then Task 15 `__all__`+CLAUDE.md). **Next: Task 16 (discipline-only ablation) → Task 17 (CUSUM rename) → Task 18 (run full 10k-stream calibration + lock pre-reg Part 2)**. Completed phases: Phase 0+1+2+3+4+5 (full detector pipeline from Trajectory → alert, synthetic generator, calibration grid). Pre-reg Part 1 LOCKED. Plan 2 spec + plan reviewer APPROVED. Branch `copytrading-drift-demo`.
