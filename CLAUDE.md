...
## Current Status
- **Copy-trading drift paper (2026-05-30)**: spec + Plan 1 both APPROVED (2 adversarial review rounds each). **Plan 1 Tasks 1-8 BUILT + committed (HEAD `d298018`): isolated `research/hyperliquid/` package, 23 tests green (`python -m pytest research/hyperliquid/tests/`), main 1374-suite untouched.** Hyperliquid data GO. Branch `copytrading-drift-demo`. Next: **Task 9 = real frozen-universe run = go/no-go** (needs Sean to assemble candidate address universe + pick T0) → Plan 2 (3-axis detector) → Plan 3 (experiments + paper → Monday-listing arXiv). Spec `docs/superpowers/specs/2026-05-30-copytrading-drift-paper-design.md`; Plan 1 `docs/superpowers/plans/2026-05-30-hyperliquid-data-pipeline.md`.
