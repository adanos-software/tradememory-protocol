...
## Current Status
- **Copy-trading drift paper (2026-05-30)**: **Plan 2 Task 16 DONE (2026-05-31)**. Discipline-only ablation complete: `mask_axes`, `drift_axes`, `ablation.py`. 154 tests green. HEAD `c2cdb3d`. **Next: Task 17 (CUSUM rename) → Task 18 (run full 10k-stream calibration + lock pre-reg Part 2)**. Completed phases: Phase 0+1+2+3+4+5+6 (full detector pipeline → ablation harness). Pre-reg Part 1 LOCKED. Branch `copytrading-drift-demo`.
