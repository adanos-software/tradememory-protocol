...
## Current Status
- **Copy-trading drift paper (2026-05-30)**: spec + Plan 1 both APPROVED (2 adversarial review rounds each). **Plan 1 Tasks 1-8 BUILT + committed (HEAD `d298018`): isolated `research/hyperliquid/` package, 23 tests green (`python -m pytest research/hyperliquid/tests/`), main 1374-suite untouched.** **Task 9 frozen-universe viability = GO** (`research/hyperliquid/COHORT-REPORT.md`): 89% raw blow-up rate on a 300-master sample BUT 66% clustered on one market-crash day → real behaviorally-driven gold = ~38-53 idiosyncratic blow-ups across ~22 independent days (grows with window/sample). **Headline cohort LOCKED: idiosyncratic lead-time (Claim A) + crash-day discrimination (Claim B).** Branch `copytrading-drift-demo`. **Pre-registration Part 1 (data/cohort) APPROVED + LOCKED 2026-05-30** (`research/hyperliquid/PRE-REGISTRATION-DRAFT.md`): T0 2025-06-01→2026-04-30, dd 0.70 (+0.5/0.85 robustness), market-event τ=3% of universe, $25k universe floor, withdrawal filter, claim gates A(lead-time>0,CI excl 0)/B(AUC-CI>0.65,FPR<0.10,PPV≥0.30)/C(no $ in abstract). **Plan 2 detector design spec WRITTEN + spec-reviewer APPROVED** (`docs/superpowers/specs/2026-05-30-copytrading-drift-detector-design.md`, commit `8f83781` + clarity fixes): 7 locked decisions (hybrid cadence, grid bucket {1h/4h/24h}, James-Stein shrinkage null, synthetic-tuned Fisher-LDA axis weights, 2-state HMM label-blind generator, legacy-CUSUM rename, define+synthetic-sanity ablation). **Awaiting Sean spec review → writing-plans** → build detector in `research/hyperliquid/detector/` (per-trade primitives → bucket-close per-axis observations → per-axis mSPRT reusing `ssrt/core.py` → Holm-min-gate + sustained-M composite → 70% guard band) → synthetic MC calibration (grid bucket/M/τ/weights/κ → Type-I≤0.02-relax-0.05 / power≥0.70) → lock pre-reg Part 2 → Plan 3 (experiments + paper → Monday-listing arXiv). Specs: detector `2026-05-30-copytrading-drift-detector-design.md`, paper `2026-05-30-copytrading-drift-paper-design.md`; Plan 1 `docs/superpowers/plans/2026-05-30-hyperliquid-data-pipeline.md`.
